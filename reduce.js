...
function vowelCount(str) {
    ...
    return strArr.reduce(function(allLetters, nextLetter){
        if (vowels.includes(nextLetter)){
            if (allLetters[nextLetter]){
                allLetters[nextLetter] += 1;
            } else {
                allLetters[nextLetter] = 1;
            }  
        } 
...
callback function at that specific value is true, the value should be placed in the first subarray. If the 
result of the callback function at that specific value is false, the value should be placed in the second subarray. 
...
function partition(arr, callback) {
    ...
    return arr.reduce(function(acc, nextVal){
        if (callback(nextVal)){
            firstArray.push(nextVal);
        } else {
            secondArray.push(nextVal);
        }
